...
instead of hostname:portnum."
  (if args
	...
	(setq server (format "%s:%d" server port)
	      byserv (cdr-safe (assoc realm (assoc server url-digest-auth-storage))))
	(cond
	 ((and prompt (not byserv))
	  (setq user (or
		      (url-do-auth-source-search server type :user)
				  ...
				  (setq retval
					(cons user
					      (url-digest-auth-create-key
					       user pass realm
					       (or url-request-method url-http-method "GET")
					       url)))))
		      url-digest-auth-storage)))
	 (byserv
	  (setq retval byserv))
	...
	(if retval
	    (progn
	      (setq retval (append retval 
				   (list (md5 (concat 
					       (or url-request-method url-http-method "GET") ":" file)))))
